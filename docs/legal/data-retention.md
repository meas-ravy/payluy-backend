...
`roadmap.md` requires two things of us: *"You will store per-merchant Bakong
...
- `api_keys.key_hash` — a one-way SHA-256 hash. The key itself is not recoverable.
...
- Implemented in `RetentionService.purgeGatewayPayloads()` (`src/retention/`).
- Driven by the `retention` BullMQ processor (repeatable job),
...
- Idempotent and dedup-keyed per day, so replicas cannot purge the same day twice
  once workers are scaled out.

Tests: `retention.service.spec.ts` — a 91-day-old payload is purged, an 89-day-old
...
a dispute or a tax audit depends on. `roadmap.md`'s "define data-retention"
...
(`signPayload(body, endpoint.secretKey)`). It **must** be recoverable, because
we have to compute a signature with it on every outbound delivery — so it cannot
be hashed like an API key.

Decision: **documented as an accepted risk for launch (P2 item in `launch-checklist.md`).** Encrypting
