...
> agreement until that review is recorded (see the P1 legal-review item in
> `launch-checklist.md`).
...
`roadmap.md` reaches the same conclusion from the CutLuy comparison: the
software-only model is defensible, but *"your contract with merchants must be
explicit that you never hold or move funds."*

## 1. The relationship

[Company name] ("we") provides software that generates payment codes and
...
**Open points:**
- The list above is drawn from `roadmap.md`, which says to mirror CutLuy's
...
- **The platform cannot currently enforce any of this.** There is no
  ...
  clause has is contract. That is a deliberate decision recorded in `launch-checklist.md`, and it
...
- **What we store, and for how long.** See `legal/data-retention.md` and the
...
- **Bakong Open API is currently unavailable to us.** It is implemented, and it is
  ...
  declined to issue. See `legal/on-behalf-of.md`.
...
6. What must the agreement say about the plaintext storage of webhook signing
   secrets (see the accepted-risk note in `legal/data-retention.md`)?
