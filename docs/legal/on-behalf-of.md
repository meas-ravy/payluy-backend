...
> **Status: OPEN.** This is the one compliance item that cannot be closed by writing
...
`roadmap.md` Phase 0, and `detection.md` §7, both ask the same thing:
...
And `roadmap.md` flags the consequence:
...
`DetectionProcessor` and `PaywayGateway`):
