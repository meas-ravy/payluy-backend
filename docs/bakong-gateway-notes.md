...
> The findings below are facts about ABA's rail, measured on 2026-09-17 with an earlier
> prototype, and apply unchanged. The NestJS names used here are defined in `architecture.md`.
...
> `uv run python -m tools.probe --config probe-config.json --report findings.json`.
...
- **Offline (`build_khqr_payload`)** — we construct the EMVCo TLV ourselves. The
  encoder is correct: a unit test (`khqr.spec.ts`) must pin our CRC-16 against a real captured
...
what the live test (`payway.live.spec.ts`, opt-in) asserts. So "initiated" and "settled" are separable,
...
- method used: **poll** (the `payment-detection` processor, plus the 30 s sweep)
...
- QR expiry behaviour on the rail: **the code lives 180 s; the checkout does
  ...
  That matters operationally. The expiry sweeper moves the row to `expired` at the deadline and
...
Gateway adapter (NestJS): `PaywayGateway` in `src/payway/` exposes
`createHostedCheckout(link, amount)` and `fetchHostedStatus({clientId, requestTime, token})`,
driven by `PaymentsService.mintQr()` and confirmed by `DetectionProcessor`.
...
- No **Bakong Open API** call was made. `BAKONG_API_TOKEN` was empty throughout.
  The detection processor skips its Bakong-credential guard when a payment has a hosted session, which is why
...
- The **attempt-history defect** surfaced by this run (only the first poll was
  ...
  a test that records every detection attempt, not only the first (`detection.processor.spec.ts`).
