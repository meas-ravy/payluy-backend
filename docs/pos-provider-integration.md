...
many merchants through the platform. One account represents the whole provider.
...
| `external_id` | *Your* merchant id, stored on the store. The platform never invents one. |
...
that merchant's own ABA PayWay account. The platform generates the QR and reports the outcome; it
...
(`["payment.completed", "payment.expired"]`); omit it or pass `["*"]` to receive all six.
...
`max_stores` is per plan: Free 1, Starter 5, Pro 50. Exceeding it returns `400 store_limit_reached`.
...
| `store` | The store's public id (UUID v4) returned when the store was created. |
...
```json
{
  ...
  "currency": "USD",
  "reference_id": "INV-2041",
  "store": "eafed232-e03f-4193-9692-cf87f2629255",
  "external_id": "merchant-alpha-77",
  "qr_string": "00020101021229…6304AB12",
  "checkout_url": "https://pay.example.com/pay/HeUWJ6IXj7oXCWFJO5sPt8fP",
...
strings in responses; `expires_at` mirrors ABA's own code lifetime (180 s observed). A payment can still settle after it — see Webhooks.
...
Statuses: `pending` → `scanned` → `paid`, plus `expired` (code withdrawn, **can still become `paid`**),
`superseded` (replacement code retired), `reversed` (refund recorded) and `failed`. `GET /v1/payments/{id}` returns the current state; `GET /v1/payments?merchant=…`
...
X-Webhook-Signature: t=1757833591,v1=8f2c…b41
```
...
`payment.superseded`, `payment.reversed`. Only `completed` and `reversed` carry `financial: true` —
`payment.expired` is **not** a lost sale; the same payment can still complete later (see `api.md` “Reporting contract”).
...
```http
GET /v1/reports/v1/payments.json?merchant=merchant-alpha-77&from=2026-09-01&to=2026-09-30
GET /v1/reports/v1/payments.csv?merchant=merchant-alpha-77&statuses=paid
```
...
`store_public_id`, `store_name` and `external_id`. CSV export is gated per plan (`csv_export_enabled`; not on Free).
