...
> **Status:** §1–§4 and §7 are the original problem statement and research plan. The chosen
> design is Strategy B against ABA's hosted checkout, polled by a 30 s sweep — see
> `architecture.md` §5 and `bakong-gateway-notes.md`. Where this file says "poll every 2–3 s" or
> "5-minute life", the architecture values (30 s sweep, ABA's 180 s code, 1 h detection window) win.
...
```
                 scan/confirm                     paid
   pending ────────────────► scanned ──────────────────► paid ──reverse──► reversed
      │                        │                          ▲
      │ ABA ttl (180 s)        │ ttl                      │ money arrives late
      ▼                        ▼                          │ (still polled for 1 h)
   expired ─────────────────────────────────────────────┘
                                                         failed (rail-confirmed failure)
   reissued code retired when the original settles ─► superseded
```
...
- `paid`: final unless reversed. Fires `payment.completed`, counts toward quota, stores Bakong txn reference.
- `expired`: **not terminal.** Fires `payment.expired` (stop showing the code). ABA accepted a
  payment 9.5 min after expiry, so detection keeps polling for `detection_window_seconds` (1 h)
  and promotes `expired → paid` when the money turns up. Expiry is driven by a scheduler that runs
...
2. **KHQR generation** — does the API expose a create-QR endpoint, or must we generate the
   TLV payload offline (port the NBC JS SDK; add a TypeScript encoder + test vectors)?
...
Record the findings in `bakong-gateway-notes.md` (create it during the spike) and pick the
