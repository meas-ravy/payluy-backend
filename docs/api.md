...
Base URL `https://pay.example.com/v1` — the API is served from the same origin as the dashboard,
...
(`/v1/admin/*`) are internal and are not part of the public surface.
...
```json
{
  ...
  "checkout_url": "https://pay.example.com/pay/PUETcMUOKStjZsCb6zAl8kg9",
  "approved_at": null,
  "created_at": "2026-09-09T12:00:00.000Z",
  "expires_at": "2026-09-09T12:03:00.000Z"
}
```
...
## POST /v1/payments/:id/reissue

Mint a fresh code for a payment whose code died. No body. Allowed only when the payment is
`expired` or `failed`; `409 payment_already_paid`, `409 payment_reversed`,
`409 payment_not_expired` otherwise. Creates a **new** payment row (new `id`, same amount,
`reference_id` and `metadata`, no `idempotency_key`) linked by `reissued_from`. If a live
successor already exists it is returned with `200` instead of minting another ABA session;
otherwise `201`. Response = the payment object with `checkout_url`.
...
- `X-Webhook-Event`: `payment.completed | payment.scanned | payment.expired |
  payment.failed | payment.superseded | payment.reversed`
- `X-Webhook-Signature`: `t=<unix>,v1=<hex>` — HMAC-SHA256 of `t.<rawBody>` with the endpoint's
...
| `csv_export_not_available` | 403 | CSV export is gated per plan on `csvExportEnabled` |
...
| `telegram_chat_id_not_set` | 400 | Store has no chat id |
| `telegram_not_configured` | 503 | Deployment has no bot token |
| `telegram_send_failed: <reason>` | 502 | Telegram refused the message |
...
`@nestjs/throttler` named throttlers and a custom tracker — API key for keyed rules, client IP otherwise):
...
replicas. With in-memory storage each replica counts separately and the effective allowance
is N times the configured value.
...
// body = the exact bytes that will be POSTed (JSON.stringify once, reuse the string)
const t = Math.floor(Date.now() / 1000);
const v1 = createHmac('sha256', endpoint.secret).update(`${t}.${body}`).digest('hex');
headers['X-Webhook-Signature'] = `t=${t},v1=${v1}`;
// receivers: timingSafeEqual on the hex, reject if |now - t| > 300 or the format is wrong.
```
...
All routes below need `Authorization: Bearer ck_live_…` **or** a dashboard session cookie,
unless marked *session only*. Everything is scoped to the caller's account; another account's id
answers `404`. Every mutation writes an audit row (see `launch-checklist.md`).

## Stores

A store is one merchant. `id` is the public id: a UUID v4.

### Store object

```json
{
  "id": "eafed232-e03f-4193-9692-cf87f2629255",
  "name": "Alpha Mart",
  "external_id": "merchant-alpha-77",
  "city": "Phnom Penh",
  "status": "active",
  "support_email": "help@alphamart.example",
  "redirect_success_url": "https://pos.example.com/paid",
  "redirect_failure_url": "https://pos.example.com/failed",
  "telegram_chat_id": null,
  "brand_color": "#0f766e",
  "logo_image_url": null,
  "whitelabel_css": null,
  "link": {
    "link_type": "aba_payway",
...
`status`: `draft` (no link) → `active` (link attached) ; `disabled` by the caller.

### Fields (create / update)

| Field | Type | Rules |
| --- | --- | --- |
| `name` | string | required on create, 1–120 |
| `external_id` | string \| null | ≤ 255, unique per account (`409 external_id_taken`) |
| `city` | string | ≤ 15 (KHQR limit), default `Phnom Penh` |
| `support_email` | string \| null | ≤ 255 |
| `redirect_success_url`, `redirect_failure_url` | string \| null | absolute https URL |
| `telegram_chat_id` | string \| null | ≤ 64 |
| `brand_color` | string \| null | `#rgb`, `#rrggbb` or `#rrggbbaa` — needs white-label |
| `logo_image_url` | string \| null | https URL — needs white-label |
| `whitelabel_css` | string \| null | `<` stripped on render — needs white-label |
| `link.raw_link` | string | required if `link` sent; must be `https://link.payway.com.kh/<slug>` |
| `link.merchant_account_id` | string | required if `link` sent; 3–120; the slug |
| `link.merchant_name` | string \| null | ≤ 120 |
...
`max_stores` per plan: Free 1, Starter 5, Pro 50.

## API keys

Key format: `ck_live_` + 32 URL-safe chars. Stored as SHA-256; the raw key is returned **once**.

```json
{
  "id": 12,
  "name": "POS production",
  "key_prefix": "ck_live_Ab3dEf9h",
  "status": "active",
  "last_used_at": null,
  "created_at": "2026-09-14T08:00:00.000Z",
  "revoked_at": null,
  "raw_key": "ck_live_Ab3dEf9h…"      // only on create and rotate
}
```

| Method | Path | Body | Success | Errors |
| --- | --- | --- | --- | --- |
| GET | `/v1/keys` | — | `200 [key]` (no `raw_key`) | — |
| POST | `/v1/keys` | `{"name": "1–64 chars"}` (no other fields) | `201` key with `raw_key` | `400 key_limit_reached`, `422` |
| POST | `/v1/keys/:id/revoke` | — | `200` key (`revoked`); idempotent | `404 key_not_found` |
| POST | `/v1/keys/:id/rotate` | — | `200` **new** key with `raw_key`; old key revoked in the same transaction | `404 key_not_found` |

Keys per plan: Free 1, Starter 3, Pro 10. A revoked key answers `401 unauthorized`.
...
{
  "id": 4,
  "url": "https://pos.example.com/payments/webhook",
...
`events`: any of `payment.completed`, `payment.scanned`, `payment.expired`, `payment.failed`,
`payment.superseded`, `payment.reversed`; `null` or `["*"]` = all.

| Method | Path | Body | Success | Errors |
| --- | --- | --- | --- | --- |
| GET | `/v1/webhooks` | — | `200 [endpoint]` | — |
| POST | `/v1/webhooks` | `{"url": "https://…", "events"?: [...]}` | `201` endpoint + `"signing_secret": "whsec_…"` (shown once) | `400 webhook_limit_reached`, `422` |
| PATCH | `/v1/webhooks/:id` | `{"url"?, "events"?, "status"?: "active"\|"disabled"}` | `200` endpoint | `404 webhook_not_found` |
| DELETE | `/v1/webhooks/:id` | — | `200` endpoint (deleted) | `404` |
| POST | `/v1/webhooks/:id/rotate-secret` | — | `200 {"id", "signing_secret"}` (old secret stops working immediately) | `404` |
| POST | `/v1/webhooks/:id/test` | — | `200 {"http_status", "response_body_preview", "headers_sent"}` — sends a synthetic signed event | `404` |
| GET | `/v1/webhooks/:id/deliveries` | `?limit=` (≤ 100) | `200 [{"delivery_id", "event_id", "event_type", "status", "http_status", "attempt_count", "last_error", "created_at", "updated_at"}]` | `404` |

Endpoints per account: 10. Delivery targets `status = active` endpoints only.

## Reports

| Method | Path | Query | Success |
| --- | --- | --- | --- |
| GET | `/v1/reports/v1/payments.json` | `from`, `to` (`YYYY-MM-DD`, inclusive, UTC), `store_id`, `merchant`, `statuses` (comma list), `page` (≥1), `per_page` (1–100, default 20) | `200 {"data": [row], "page", "per_page", "total"}` |
| GET | `/v1/reports/v1/payments.csv` | same, no paging (streamed) | `200 text/csv` |

Row / CSV columns: `public_id, status, amount, currency, reference_id, store_public_id,
store_name, external_id, created_at, paid_at, reversed_at, bank_ref`.

Errors: `400 invalid_date`, `404 store_not_found`, `403 csv_export_not_available` (CSV only, plan-gated).

## Billing (*session only*)

| Method | Path | Body | Success | Errors |
| --- | --- | --- | --- | --- |
| GET | `/v1/billing/plans` | — | `200 [plan]` (public, active plans) | — |
| GET | `/v1/billing/subscription` | — | `200 {"subscription": {...} \| null, "plan": plan \| null}` | — |
| POST | `/v1/billing/change-plan` | `{"plan_code": "starter"}` | `200 {"subscription": {...}}` | `404 plan_not_found`, `400 plan_not_available` |
| GET | `/v1/billing/invoices` | — | `200 [invoice]` | — |
| POST | `/v1/billing/invoices/:id/v1/khqr` | — | `201` a payment object for paying the invoice by KHQR (dogfooding) | `404 invoice_not_found`, `400 invoice_already_paid` |

Plan object: `code, name, monthly_fee_cents, payments_included, max_stores, max_keys,
max_webhooks, csv_export_enabled`. `GET /v1/billing/plans` is also used unauthenticated by the
pricing page.
